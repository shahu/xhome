...
void ReconfigureSignalHandler(void);
void ResetStreamSignalHandler(void);
bool Initialize();
void Run();
void Cleanup();
void PrintHelp();
...
int main(int argc, char **argv) {
	...
	do {
		...
		if (gRs.resetStream || Initialize()) {
            gRs.resetStream = false;
			Run();
		} else {
			gRs.run = false;
		}
		if( gRs.pConfigFile->IsDaemon() && gRs.daemon != false ) {
		  gRs.run = true;
		}
        if(gRs.resetStream || gRs.reconfigure){
            gRs.run = true;
        }
		else{
			//5. Cleanup
...
bool Initialize() {
	...
	if(gRs.reconfigure){
		gRs.reconfigure = false;
		return gRs.pConfigFile->ReconfigApplications(configFilePath);
	}
	else {
	}
	Logger::Init();
	...
	//installConfRereadSignal(ReconfigureSignalHandler);
	installUSR1Signal(ReconfigureSignalHandler);
	installUSR2Signal(ResetStreamSignalHandler);
	return true;
}
...
void Run() {
	...
	while (!gRs.reconfigure && IOHandlerManager::Pulse()) {
...
void ReconfigureSignalHandler(void){
	WARN("reconfigrue");
	gRs.reconfigure = true;
}

void ResetStreamSignalHandler(void){
    WARN("reset stream");
    gRs.resetStream = true;
    StreamMonitor::ResetError();
}
...
pid_t ReadPidFile() {
	...
	pid_t pid = -1;
        if (stat(STR(pidFile), &sb) == 0) {
		File f;
		if (!f.Initialize(STR(pidFile), FILE_OPEN_MODE_READ)) {
			WARN("Unable to open PID file %s", STR(pidFile));
			return pid;
		}
		else{
			int32_t id = -1;
			if (!f.ReadI32(&id)) {
				WARN("Unable to read PID from file %s", STR(pidFile));
			}
			else{
				pid=(pid_t)id;
			}

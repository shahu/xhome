...
protected:
	...
	double _lastRecvTimestamp;
	double _lastSentTimestamp;
	double _lastRecvSpeed;
	double _lastSentSpeed;
	uint32_t _lastTotalSentBytes;
	uint32_t _lastTotalRecvBytes;
	uint32_t _speedSample;
...
public:
	...
	void OnRecvAmount(uint32_t amount);
	void OnSendAmount(uint32_t amount);
	double GetLastSentSpeed(){ return _lastSentSpeed;};
	double GetLastRecvSpeed(){ return _lastRecvSpeed;};
	void OnBroken(int type);
	void OnAvailable(int type);
	void SetIdleBroken(bool state){_idleBroken = state;};
	bool GetIdleBroken(){ return _idleBroken;};
	const string& GetCurrentStreamName() const { return _currentStreamName;};
private:
	//utility function
	string ToString(uint32_t currentId);
	inline void SpeedAmount(uint32_t amount,bool upstream);
};

...
bool InNetLiveFLVStream::FeedData(uint8_t *pData, uint32_t dataLength,
	...
	if (isAudio) {
		...
		if (absoluteTimestamp < _lastAudioTime) {
			WARN("_lastAudioTime:%.2f, absoluteTimestamp:%.2f", _lastAudioTime, absoluteTimestamp);
			double new_absoluteTimestamp = (absoluteTimestamp + (_audioTurnCount * 0x10000000));
			if (new_absoluteTimestamp < _lastAudioTime) {
				new_absoluteTimestamp += 0x10000000;
				
				// In flv, the timestape is SInt32
				if (new_absoluteTimestamp - _lastAudioTime < 1000) { 
					 _audioTurnCount += 1;
				} else {
					new_absoluteTimestamp = _lastAudioTime;
				}				
				WARN("new_absoluteTimestamp:%.2f, _audioTurnCount:%ld", new_absoluteTimestamp, _audioTurnCount);
			}
			absoluteTimestamp = new_absoluteTimestamp;
		}
		...
		if (absoluteTimestamp < _lastVideoTime) {
			WARN("_lastVideoTime:%.2f, absoluteTimestamp:%.2f", _lastVideoTime, absoluteTimestamp);
			double new_absoluteTimestamp = (absoluteTimestamp + (_videoTurnCount * 0x10000000));
			if (new_absoluteTimestamp < _lastVideoTime) {
				new_absoluteTimestamp += 0x10000000;
				
				// In flv, the timestape is SInt32
				if (new_absoluteTimestamp - _lastVideoTime < 1000) { 
					 _videoTurnCount += 1;
				} else {
					new_absoluteTimestamp = _lastVideoTime;
				}				
				WARN("new_absoluteTimestamp:%.2f, _videoTurnCount:%ld", new_absoluteTimestamp, _videoTurnCount);
			}
			absoluteTimestamp = new_absoluteTimestamp;
		}

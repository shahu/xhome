...
bool InNetRTMPStream::FeedData(uint8_t *pData, uint32_t dataLength,
	...
	if (isAudio) {
		...
		if (absoluteTimestamp < _lastAudioTime) {
			...
			absoluteTimestamp = _lastAudioTime;
		}
		...
		if (absoluteTimestamp < _lastVideoTime) {
			...
			absoluteTimestamp = _lastVideoTime;
		}
		_lastVideoTime = absoluteTimestamp;
	}
	LinkedListNode<BaseOutStream *> *pTemp = _pOutStreams;
	if(! pTemp){
		Variant tmpConfig = GetProtocol()->GetCustomParameters()["customParameters"]["externalStreamConfig"];
		if(tmpConfig == V_MAP && tmpConfig.HasKey("maxIdle")){
			//INFO("yes has maxIdle%d",(int)tmpConfig["maxIdle"]);
			if(_startIdle == 0){
				_startIdle = time(NULL);
			}
			else if( time(NULL) - _startIdle >= (int)tmpConfig["maxIdle"]){
				WARN("max idle");
				_pProtocol->SetIdleBroken(true);	
				return false;
			}
		}

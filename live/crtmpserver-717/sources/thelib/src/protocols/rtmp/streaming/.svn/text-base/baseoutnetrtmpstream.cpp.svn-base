...
#define ALLOW_EXECUTION2(feederProcessed,dataLength,isAudio,frameLevel) if (!AllowExecution2(feederProcessed,dataLength,isAudio,frameLevel)) { /*FINEST("We are not allowed to send this data");*/ return true; }
...
: BaseOutNetStream(pProtocol, pStreamsManager, type, name) {
	...
	_lastIDropped = _lastPDropped = false;
	...
	if (isAudio) {
		...
		if (_isFirstAudioFrame) {
			...
			if ((*_pDeltaAudioTime) > absoluteTimestamp) {
				//AAC sequence header, set timestamp to 0
				if (pData[0] >> 4 == 10 && pData[1] == 0) {
					(*_pDeltaAudioTime) = absoluteTimestamp;
				} else {
					FINEST("A|WAIT: (*_pDeltaAudioTime):%.2f, absoluteTimestam:%.2f", (*_pDeltaAudioTime), absoluteTimestamp);
					_pRTMPProtocol->EnqueueForOutbound();
					return true;
				}
			}
...
//			ALLOW_EXECUTION(processedLength, dataLength, isAudio);
			ALLOW_EXECUTION2(processedLength, dataLength, isAudio, 0);
			H_IA(_audioHeader) = false;
			if (processedLength == 0) {
				H_TS(_audioHeader) = (uint32_t) ((absoluteTimestamp - (*_pDeltaAudioTime) + _seekTime)
					- _pChannelAudio->lastOutAbsTs);
				if ( absoluteTimestamp - (*_pDeltaAudioTime) + _seekTime < _pChannelAudio->lastOutAbsTs ) {
					FINEST("A: absoluteTimestamp:%.2f, _pDeltaAudioTime:%.2f, _seekTime:%.2f, lastOutAbsTs:%.2f", 
						absoluteTimestamp, (*_pDeltaAudioTime), _seekTime, _pChannelAudio->lastOutAbsTs);
				}
			}
		}
		...
		{	
			_videoPacketsCount++;
			_lastFrameLevel = CalcFrameLevel(pData, dataLength, false);
		}
		...
		if (_isFirstVideoFrame) {
			...
			if ((*_pDeltaVideoTime) > absoluteTimestamp) {
				FINEST("V|WAIT: (*_pDeltaVideoTime):%.2f, absoluteTimestam:%.2f", (*_pDeltaVideoTime), absoluteTimestamp);
				_pRTMPProtocol->EnqueueForOutbound();
				return true;
			}
...
//			ALLOW_EXECUTION(processedLength, dataLength, isAudio);
			ALLOW_EXECUTION2(processedLength, dataLength, isAudio, _lastFrameLevel);
			H_IA(_videoHeader) = false;
			if (processedLength == 0) {
				H_TS(_videoHeader) = (uint32_t) ((absoluteTimestamp - (*_pDeltaVideoTime) + _seekTime)
					- _pChannelVideo->lastOutAbsTs);
				if ( absoluteTimestamp - (*_pDeltaVideoTime) + _seekTime < _pChannelVideo->lastOutAbsTs ) {
					FINEST("V: absoluteTimestamp:%.2f, _pDeltaVideoTime:%.2f, _seekTime:%.2f, lastOutAbsTs:%.2f", \
							absoluteTimestamp, (*_pDeltaVideoTime), _seekTime, _pChannelVideo->lastOutAbsTs);
				}
			}
		}
...
// Added by Tady, 07122012
bool BaseOutNetRTMPStream::AllowExecution2(uint32_t totalProcessed, uint32_t dataLength, bool isAudio, uint32_t frameLevel) {
	if (!_canDropFrames) {
		// we are not allowed to drop frames
		return true;
	}
...
*/
	...
	do 
	{
		...
		uint32_t blockedBufferSize = _pRTMPProtocol->GetOutputBuffer() == NULL
			? 0 : GETAVAILABLEBYTESCOUNT(*_pRTMPProtocol->GetOutputBuffer());
		if (frameLevel == 0
			&& blockedBufferSize > _maxBufferSize * 2 * fadeoutRate)
		{
			dropIt = true;
			DEBUG("Drop 0!!!");
			break;
			...
			&& blockedBufferSize > _maxBufferSize * 1 * fadeoutRate)
		{
			_lastIDropped = true;
			dropIt = true;
			DEBUG("Drop I !! [%f]", (float)blockedBufferSize / _maxBufferSize);
			break;
		}
		else if (frameLevel == 2 
			&& blockedBufferSize > _maxBufferSize * 0.8 * fadeoutRate)
		{
			_lastPDropped = true;
			dropIt = true;
			DEBUG("Drop P ! [%f]", (float)blockedBufferSize / _maxBufferSize);
			break;
		}
		else if (frameLevel == 3
			&& blockedBufferSize > _maxBufferSize * 0.3 * fadeoutRate)
		{
			dropIt = true;
			DEBUG("Drop B  [%f]", (float)blockedBufferSize / _maxBufferSize);
			break;
		}
...
uint32_t BaseOutNetRTMPStream::CalcFrameLevel( uint8_t *pData, uint32_t length, bool isAudio )
{
	uint32_t frameLevel = 0;
	if (!isAudio && (pData[0] & 0xF) == 0x7) //AVC codec.
	{ 
		Bitstream bs;
		bs.read_len = 0;
		bs.frame_bitoffset = 0;
		bs.bitstream_length = bs.code_len = length - 5; // Data len before Nalu is 5.
		bs.streamBuffer = (byte *)pData + 5; // Data len before Nalu.

		if (pData[1] == 0)
		{ //codec setup.
			_nal.parse_AVCDecoderConfigurationRecord(&bs);
			if (_nal.is_ready_)
				DEBUG("AVC Config Ready.");
			else
				WARN("Parse AVC Configuration Failed!!");
			
		}
		else if (pData[1] == 1)
		{
			_nal.parse_without_startcode(&bs);
			if (_nal.is_ready_)
			{
				// printf("slice_type [%d] \n", _nal.slice_type);
				if (_nal.slice_type == 6)
				{
					frameLevel = 3;
				}
				else if (_nal.slice_type == 5)
				{
					frameLevel = 2;
				}
				else if (_nal.slice_type == 7)
				{
					frameLevel = 1;
				}
			}
			else
				WARN("Parse NALU failed!!");
		}
	}

...
bool BaseRTMPAppProtocolHandler::PullExternalStream(URI uri, Variant streamConfig) {
	...
	if (scheme == "rtmp") {
	 ...
	 INFO("PullExternalStream stream name `%s`. url `%s`",
                        STR(localStreamName), STR(uri.originalUri()));
...
bool BaseRTMPAppProtocolHandler::InboundMessageAvailable(BaseRTMPProtocol *pFrom,
	...
	if (!_rtmpProtocolSerializer.Deserialize(header, inputBuffer, request)) {
		FATAL("Unable to deserialize message,%s,%s",STR(inputBuffer),STR(request.ToString()));
		return false;
	}
...
bool BaseRTMPAppProtocolHandler::ProcessInvokePlay(BaseRTMPProtocol *pFrom,
	...
	switch ((int32_t) startTime) {
		case -2: //live or recorded
		{
			...
			Variant tmpConfig = _upstream;	
			string upstream = tmpConfig == V_MAP && tmpConfig.HasKey("upstream")?(string)tmpConfig["upstream"]:"";
			const int retries = upstream.empty() || !tmpConfig.HasKey("retries") ? 1 : (int) tmpConfig["retries"];
			int retried = upstream.empty()?retries:0;
			URI uri;
			if (retried == 0){
				URI::FromString(upstream+streamName,true,uri);
				tmpConfig["uri"]=uri;
			}
			do{
				INFO("try to link to live stream");
				...
				if (linked)
					return true;
				INFO("try to link to file stream");
				...
				if( ++retried>retries )
					break;
				tmpConfig["localStreamName"]=streamName;
				INFO("try connect for pull push stream name `%s`. url `%s`",STR(streamName), STR(uri.originalUri()));
			}	
			while(PullExternalStream(uri,tmpConfig));

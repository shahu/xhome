...
bool BaseRTMPProtocol::SignalInputData(IOBuffer &buffer) {
	if (_enqueueForDelete)
		return true;
	//INFO("rtmp streamName :%s,protocol:%p",STR(_currentStreamName),this);
	bool result = false;
...
InNetRTMPStream * BaseRTMPProtocol::CreateINS(uint32_t channelId,
	...
	if( _currentStreamName.empty() ){
		_currentStreamName =  streamName;
	}
	GetFarEndpoint()->OnAvailable(0);
	return pStream;
}
	...
	while (true) {
		...
		if (channel.state == CS_PAYLOAD ) {
			...
			switch (H_MT(header)) {
				...
				default:
				{
					if (!channel.inputData.ReadFromInputBuffer(buffer, tempSize) )
						continue;
					channel.lastInProcBytes += tempSize;
					if (!buffer.Ignore(tempSize)) {
					...
					if (H_ML(header) == channel.lastInProcBytes) {
						...
						if (!_pProtocolHandler->InboundMessageAvailable(this, header, channel.inputData)) {
							FATAL("Unable to send rtmp message to application,stream:%s",STR(_currentStreamName));
							return false;
						}
						_rxInvokes++;

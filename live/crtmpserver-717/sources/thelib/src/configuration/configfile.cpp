...
bool ConfigFile::ReconfigApplications(const string& path) {
	Variant config;
	if (!ReadLuaFile(path, CONF_CONFIGURATION, config)) {
		FATAL("Unable to reread configuration file: %s", STR(path));
		return false;
	}
	if (!config.HasKeyChain(V_MAP, false, 1, CONF_APPLICATIONS)) {
                WARN("No applications specified");
                return true;
        }
        Variant temp = config.GetValue(CONF_APPLICATIONS, false);
		...
		map<string,Module>::iterator itr=_modules.find(MAP_VAL(i)[CONF_APPLICATION_NAME]);
		if( itr != _modules.end() ){
			itr->second.ReconfigApplication(MAP_VAL(i));
		}
        }
	return true;
}
...
bool ConfigFile::Normalize() {
	if(!NormalizeStreamMonitor()){
		FATAL("Unable to normalize stream monitor");
		return false;
	}
...
bool ConfigFile::NormalizeStreamMonitor() {
	if (!_configuration.HasKeyChain(V_MAP, false, 1, "streamMonitor")) {
		WARN("No stream monitor specified");
		return true;
	}
	Variant temp = _configuration.GetValue("streamMonitor", false);

	if( temp != V_MAP ){
		WARN("Invalid streamMonitor config");
	}
	else{
		return StreamMonitor::Init(temp);
	}
	return true;
}

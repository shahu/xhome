...
bool BaseClientApplication::OutboundConnectionFailed(Variant &customParameters) {
	if (customParameters["customParameters"].HasKey("externalStreamConfig")){
		string streamName,streamType;
		if(customParameters["customParameters"]["externalStreamConfig"].HasKey("uri")){
			streamType = (string)customParameters["customParameters"]["externalStreamConfig"]["uri"]["scheme"];
		}
		if(customParameters["customParameters"]["externalStreamConfig"].HasKey("localStreamName")){
			streamName = (string)customParameters["customParameters"]["externalStreamConfig"]["localStreamName"];
		}
		if(!streamName.empty() && ! streamType.empty())
			StreamMonitor::OnStatus(streamName,streamType,StreamMonitor::StreamBroken);
	}
...
bool BaseClientApplication::PullExternalStream(Variant streamConfig) {
	if(streamConfig.HasKey("upstream") && streamConfig["upstream"] == V_STRING){
		INFO("upstream:%s",STR(streamConfig["upstream"]));
		//2. Split the URI
		URI uri;
		if (!URI::FromString(streamConfig["upstream"], true, uri)) {
		...
		pProtocolHandler->SetUpstream(streamConfig);
		return true;
	}
	//1. Minimal verification
	else if (streamConfig["uri"] != V_STRING) {
		FATAL("Invalid uri");
		return false;
	}
...
bool BaseClientApplication::Reconfigure(Variant appConfig){

	if (appConfig["externalStreams"] == V_NULL) {
		/*FOR_MAP(_streamsManager.GetAllStreams(),uint32_t,BaseStream *,k){
			//MAP_VAL(k)->GetProtocol()->SetApplication(NULL);
			//MAP_VAL(k)->GetProtocol()->EnqueueForDelete();
			MAP_VAL(k)->EnqueueForDelete();	
		}*/
		return true;
	}

	if (appConfig["externalStreams"] != V_MAP) {
		FATAL("Invalid rtspStreams node");
		return false;
	}
		
	_configuration["externalStreams"] = appConfig["externalStreams"];
	WARN("reload externalStreams");
	set<string> newStreams,oldStreams,discardStreams,addStreams;

	Variant streamConfigs;
	streamConfigs.IsArray(false);

	FOR_MAP(appConfig["externalStreams"], string, Variant, i) {
		Variant &temp = MAP_VAL(i);
		if ((!temp.HasKeyChain(V_STRING, false, 1, "localStreamName"))
				|| ((string) temp.GetValue("localStreamName", false) == "")) {
			WARN("External stream configuration is doesn't have localStreamName property invalid:\n%s",
		...
		string localStreamName = (string) temp.GetValue("localStreamName", false);
		newStreams.insert(localStreamName);
		streamConfigs[localStreamName] = temp;
	}
	map<uint32_t, BaseStream *> allStreams = _streamsManager.GetAllStreams();
	FOR_MAP(allStreams,uint32_t,BaseStream *,j){
		if(MAP_VAL(j) && !MAP_VAL(j)->GetName().empty() && TAG_KIND_OF(MAP_VAL(j)->GetType(),ST_IN) ){
			oldStreams.insert(MAP_VAL(j)->GetName());
		}
	}
	set_difference(newStreams.begin(),newStreams.end(),oldStreams.begin(),oldStreams.end(),inserter(addStreams,addStreams.begin()));
	set_difference(oldStreams.begin(),oldStreams.end(),newStreams.begin(),newStreams.end(),inserter(discardStreams,discardStreams.begin()));

	FOR_MAP(streamConfigs, string, Variant, i) {
		string localStreamName = (string) MAP_KEY(i);
		INFO("check streamname:%s",STR(localStreamName));
		if( addStreams.find(localStreamName) != addStreams.end() ){
			//append new stream
			INFO("append new stream:%s",STR(localStreamName));
			//streamConfigs[localStreamName] = temp;
			Variant &streamConfig = MAP_VAL(i);
			if (!PullExternalStream(streamConfig)) {
				WARN("External stream configuration is invalid:\n%s",
	...
	for(set<string>::iterator itr=discardStreams.begin();itr!=discardStreams.end();itr++){
		INFO("remove stream:%s",STR(*itr));
		FOR_MAP(allStreams,uint32_t,BaseStream *,k){
			if( MAP_VAL(k)->GetName() == *itr ){	
				//MAP_VAL(k)->GetProtocol()->SetApplication(NULL);
				//MAP_VAL(k)->GetProtocol()->EnqueueForDelete();
				MAP_VAL(k)->EnqueueForDelete();	
			}
